...
public class PostmastersDeskPostMenu extends AbstractContainerMenu {
    public final PostmastersDeskBlockEntity blockEntity;
    private final Level level;

    public PostmastersDeskPostMenu(int containerId, Inventory inv, FriendlyByteBuf extraData) {
        this(containerId, inv, inv.player.level().getBlockEntity(extraData.readBlockPos()));
    }

    public PostmastersDeskPostMenu(int containerId, Inventory inv, BlockEntity blockEntity) {
    ...
    private void addPostInventory(ItemStackHandler inv) {
        for (int i = 0; i < 3; ++i) {
            for (int l = 0; l < 9; ++l) {
                this.addSlot(new SlotItemHandler(inv, l + i * 9, 8 + l * 18, 25 + i * 18));
            }
        }
    }
